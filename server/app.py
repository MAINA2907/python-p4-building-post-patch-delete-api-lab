...
def bakery_by_id(id):
    ...
    bakery_s = Bakery.query.filter_by(id=id).first()

    if request.method == "GET":
     bakery_serialized = bakery_s.to_dict()
     return make_response ( bakery_serialized, 200  )
    
    elif request.method == "PATCH":
        for attr in request.form:
            setattr(bakery_s, attr,request.form[attr])

        db.session.add(bakery_s)
        db.session.commit()
        response = make_response(bakery_s.to_dict(), 200)
        return response
